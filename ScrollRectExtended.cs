...
public class ScrollRectExtended : UIBehaviour, IInitializePotentialDragHandler, IEventSystemHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup, ILayoutController
{
	...
	private float _check_timer_interval;
	...
	public virtual void OnBeginDrag(PointerEventData eventData)
	{
		...
		if ((int)eventData.button == 0 && ((UIBehaviour)this).IsActive())
		{
			UpdateBounds();
			m_PointerStartLocalCursor = Vector2.zero;
	...
	public virtual void OnDrag(PointerEventData eventData)
	{
		...
		if ((int)eventData.button != 0 || !((UIBehaviour)this).IsActive() || !RectTransformUtility.ScreenPointToLocalPointInRectangle(viewRect, eventData.position, eventData.pressEventCamera, ref val))
		{
			return;
		}
		UpdateBounds();
	...
	public static void SendMessageToAll(string pMessage, PointerEventData pEventData)
	{
		for (int i = 0; i < instances.Count; i++)
		{
			((Component)instances[i]).SendMessage(pMessage, (object)pEventData);
		}
	}
}
